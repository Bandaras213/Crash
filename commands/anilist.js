...
module.exports = async (bot, message, args, Discord) => {
  ...
  let anilistid = await UserlistDBobj.userlist[indexuserdiscid].anilistid;
  let mentionanilistid
  if (mention) {
    let indexmentiondiscid = UserlistDBobj.userlist.findIndex(did => did.discid == mention.id);
    if (indexmentiondiscid != -1) {
      mentionanilistid = await UserlistDBobj.userlist[indexmentiondiscid].anilistid;
    } else {
      mentionanilistid = undefined
    }
  }
  let aniliststringname
  if (args[0] != "save") {
    let indexstringname = UserlistDBobj.userlist.findIndex(did => did.anilistusername == args.join(" "));
    if (indexstringname != -1) {
      aniliststringname = await UserlistDBobj.userlist[indexstringname].anilistusername;
    } else {
      aniliststringname = undefined
    }
  }
      ...
      try {
        await fetch("https://graphql.anilist.co", {
          ...
          .then(async fetch1 => {
            ...
            await message.channel.send(`${user}, ${username} (${userID}) is ${username}'s Anilist Profile!`, { embed });
          });
      } catch (error) {
        return message.channel.send(`${user}, Couldn't find a User with the name "**${args.join(" ")}**"! Please check your spelling and try again!`);
      }
    }
