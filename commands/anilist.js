...
module.exports = async (bot, message, args, Discord) => {
  ...
  if (anilistid == undefined && !mention && args.length == 0 && args[0] != "save") {
    return message.channel.send(`${user}, Looks like you don't have a Anilist! Save a Anilist by using €anilist save [name]!`);
  };

  if (mentionanilistid == undefined && mention && args[0] != "save") {
    return message.channel.send(`${user}, Looks like ${mention} doesn't have a Anilist!`);
  };

  if (aniliststringname == undefined && !mention && args.length > 0 && args[0] != "save") {
    return message.channel.send(`${user}, Looks like "**${args.join(" ")}**" doesn't have a anilist! Tell them to save a anilist by using €anilist save [name]!`);
  };
