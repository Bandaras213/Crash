...
var value1 = [`Reaction: ${emoji[1]}`, `Reaction: ${emoji[2]}`, `Reaction: ${emoji[3]}`, `Reaction: ${emoji[4]}`, `Reaction: ${emoji[5]}`, `Reaction: ${emoji[6]}`, `Reaction: ${emoji[7]}`];
...
module.exports = async (bot, message, args, Discord) => {
  ...
  if (finduserdiscid && mention == undefined && args[0] != "save") {
    ...
    waifuid = await WaifuDBobj.waifus[indexuserdiscid].waifuid;
    waifuname = await WaifuDBobj.waifus[indexuserdiscid].waifuname;
    ...
    await fetch("https://graphql.anilist.co", {
      ...
      .then(async fetch1 => {
        ...
        let description = fetch1.data.Page.characters[0].description;
        let isindatas = fetch1.data.Page.characters[0].media.edges;
        let siteUrl = fetch1.data.Page.characters[0].media.edges;
        let format = fetch1.data.Page.characters[0].media.edges;
  ...
  if (findmentiondiscid && args[0] != "save") {
    ...
    waifuid = WaifuDBobj.waifus[indexmentiondiscid].waifuid;
    waifuname = WaifuDBobj.waifus[indexmentiondiscid].waifunname;
