...
module.exports = async (bot, message, args, Discord) => {
  ...
  if (args[0] == "save") {
    ...
    await fetch("https://kitsu.io/api/edge/users?filter[name]=" + `${usersearchname}`, {
      method: "GET",
      headers: {
      ...
      .then(async fetch1 => {
        ...
        await message.channel.send(`${user}, ${usersearchname} (${userID}) is now your Kitsu!`);
    ...
    if (!kitsustringname && !mention && args.length > 0) {
      try {
        await fetch("https://kitsu.io/api/edge/users?filter%5Bname%5D=" + userstring, {
          method: "GET",
          headers: {
            "Content-Type": "application/vnd.api+json",
            Accept: "application/vnd.api+json"
        ...
        return message.channel.send(`${user}, Couldn't find a User with the name "**${userstring}**"! Please check your spelling and try again!`);
      };
    };

    if (kitsuid && !mention && args.length == 0) {
      await fetch("https://kitsu.io/api/edge/users?filter%5Bid%5D=" + kitsuid, {
        method: "GET",
        headers: {
          "Content-Type": "application/vnd.api+json",
          Accept: "application/vnd.api+json"
        ...
        .then(async fetch1 => {
          if (fetch1.data.length == 0) {
            return message.channel.send(`${user}, Couldn't find a matching Kitsulist for '**${user}**'`);
          }
    ...
    if (mentionkitsuid && mention) {
      await fetch("https://kitsu.io/api/edge/users?filter%5Bid%5D=" + mentionkitsuid, {
        method: "GET",
        headers: {
          "Content-Type": "application/vnd.api+json",
          Accept: "application/vnd.api+json"
        ...
        .then(async fetch1 => {
          if (fetch1.data.length == 0) {
            return message.channel.send(`${user}, Couldn't find a matching Kitsulist for '**${mention}**'`);
          }
    ...
    if (kitsustringname && !mention && args.length > 0) {
      await fetch("https://kitsu.io/api/edge/users?filter%5Bname%5D=" + kitsustringname, {
        method: "GET",
        headers: {
          "Content-Type": "application/vnd.api+json",
          Accept: "application/vnd.api+json"
